...
class SingleGaitModelDesen():
	...
	def build_or_load(self, input_shape, number_convolutional_layers, filters_size, filters_numbers, strides,
					  ...
					  activation_fun='relu', pruning=False, frequency = 100, begin_step = 2000, target_sparsity = 0.9,
					  from3dto2d=False, batchsize=150):
	...
	def build(self, input_shape, number_convolutional_layers, filters_size, filters_numbers, strides, ndense_units=512,
		...
		if use3D:
			if model_version == 'bmvc':
				convBranch = self.build_3Dbranch_Manuel("convBranch", input, input_shape, ndense_units, pruning,
			...
			if model_version == 'bmvc':
				if lstm >= 8:
					...
					convBranch = self.build_branch_Manuel("convBranch", input, input_shape, number_convolutional_layers, filters_size,
...
#			output = tf.keras.layers.Dense(ndense_units, name="code")(main_branch)
			...
			#input.add(convBranch)
			print("")

		#output = tf.keras.layers.Dense(ndense_units, name="code")(main_branch)
		convBranch = tf.keras.layers.Dense(ndense_units, name="code")(convBranch) #name2d -> code_1
...
#			main_branch = output
	...
	def build_branch_Manuel(self, name, input, input_shape=(50, 60, 60), number_convolutional_layers=4, filters_size=None,
		...
		if lstm == 2:
			...
			if lstm == 1:
				...
				if activation_fun == 'leaky':
					...
					conv_1 = tf.keras.layers.Conv2D(filters_numbers[0], kernel_size=(filters_size[0]), strides=(strides[0]),
				                                    kernel_regularizer=L2_norm, activation='relu', input_shape=input_shape,
				                                    data_format='channels_first')(input)

			max_1 = tf.keras.layers.MaxPooling2D(pool_size=(2, 2), data_format='channels_first')(conv_1)
			...
			'''for i in range(1, number_convolutional_layers):
				if activation_fun == 'leaky':
					...
					conv_2 = tf.keras.layers.Conv2D(filters_numbers[i], kernel_size=(filters_size[i]), strides=(strides[i]),
						                       kernel_regularizer=L2_norm, activation='relu',
						                       data_format='channels_first')(max_1)'''
			conv_2 = tf.keras.layers.Conv2D(filters_numbers[1], kernel_size=(filters_size[1]), strides=(strides[1]),
											kernel_regularizer=L2_norm, activation='relu',
											data_format='channels_first')(max_1)

			#if i != number_convolutional_layers - 1:
			max_2 = tf.keras.layers.MaxPooling2D(pool_size=(2, 2), data_format='channels_first')(conv_2)

			conv_3 = tf.keras.layers.Conv2D(filters_numbers[2], kernel_size=(filters_size[2]), strides=(strides[2]),
											kernel_regularizer=L2_norm, activation='relu',
											data_format='channels_first')(max_2)
			max_3 = tf.keras.layers.MaxPooling2D(pool_size=(2, 2), data_format='channels_first')(conv_3)

			conv_4 = tf.keras.layers.Conv2D(filters_numbers[3], kernel_size=(filters_size[3]), strides=(strides[3]),
											kernel_regularizer=L2_norm, activation='relu',
											data_format='channels_first')(max_3)
			max_4 = tf.keras.layers.MaxPooling2D(pool_size=(2, 2), data_format='channels_first')(conv_4)

			flatten = tf.keras.layers.Flatten(name="flatten")(max_4)

			if add_dense:
				# Add dense layer + dropout
				dense_1 = tf.keras.layers.Dense(ndense_units * 2, name="dense")(flatten)
				if dropout > 0:
					drop = tf.keras.layers.Dropout(dropout, name="drop")(dense_1)
	...
	def build_3Dbranch_Manuel(self, name, input, input_shape=(25, 60, 60, 1), ndense_units=512, pruning=False,
							frequency = 100, begin_step = 2000,target_sparsity = 0.9):

		#convBranch = tf.keras.Sequential(name=name)
		#input = tf.keras.layers.InputLayer(input_shape=input_shape, name='input')

		conv_1 = tf.keras.layers.Conv3D(64, (3, 5, 5), strides=(1, 2, 2), padding='valid', activation='relu',
		                        input_shape=input_shape, data_format='channels_last')(input)

		conv_2 = tf.keras.layers.Conv3D(128, (3, 3, 3), strides=(1, 2, 2), padding='valid', activation='relu',
		                         data_format='channels_last')(conv_1)

		conv_3 = tf.keras.layers.Conv3D(256, (3, 3, 3), strides=(2, 2, 2), padding='valid', activation='relu',
		                         data_format='channels_last')(conv_2)

		conv_4 = tf.keras.layers.Conv3D(512, (3, 3, 3), strides=(2, 2, 2), padding='valid', activation='relu',
		                         data_format='channels_last')(conv_3)

		conv_5 = tf.keras.layers.Conv3D(512, (3, 2, 2), strides=(1, 1, 1), padding='valid', activation='relu',
		                         data_format='channels_last')(conv_4)

		conv_6 = tf.keras.layers.Conv3D(512, (2, 1, 1), strides=(1, 1, 1), padding='valid', activation='relu',
		                         data_format='channels_last')(conv_5)

		# Dense without activation function
		conv_7 = tf.keras.layers.Conv3D(ndense_units, (1, 1, 1), strides=(1, 1, 1), activation=None,
		                        kernel_regularizer=tf.keras.regularizers.l2(1e-3),
		                        kernel_initializer='he_uniform', name="grayCode")(conv_6)

		flatten = tf.keras.layers.Flatten()(conv_7)
