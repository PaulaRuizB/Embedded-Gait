...
int energy_measure(energy_params param, float &energy, float &maxenergy, bool &end_inference)
{	
	std::string file_path;
    	if (param.device == "xavier"){
		if (param.sensor == "gpu"){
			file_path = "/sys/bus/i2c/drivers/ina3221/1-0040/hwmon/hwmon3/curr1_input";
		}
		else if (param.sensor == "cpu"){
			file_path = "/sys/bus/i2c/drivers/ina3221/1-0040/hwmon/hwmon3/curr2_input";
		}
		else if (param.sensor == "soc"){
			file_path = "/sys/bus/i2c/drivers/ina3221/1-0040/hwmon/hwmon3/curr3_input";
		}
		else if (param.sensor == "cv"){
			file_path = "/sys/bus/i2c/drivers/ina3221/1-0041/hwmon/hwmon4/curr1_input";
		}
		else if (param.sensor == "vddrq"){
			file_path = "/sys/bus/i2c/drivers/ina3221/1-0041/hwmon/hwmon4/curr2_input";
		}
		else if (param.sensor == "sys5v"){
			file_path = "/sys/bus/i2c/drivers/ina3221/1-0041/hwmon/hwmon4/curr3_input";
		}
	}
	else if (param.device == "nano"){
		if (param.sensor == "POM_5V_IN"){
			file_path = "/sys/bus/i2c/drivers/ina3221x/6-0040/iio:device0/in_current0_input";
		}
		else if (param.sensor == "gpu"){
			file_path = "/sys/bus/i2c/drivers/ina3221x/6-0040/iio:device0/in_current1_input";
		}
		else if (param.sensor == "POM_5V_CPU"){
			file_path = "/sys/bus/i2c/drivers/ina3221x/6-0040/iio:device0/in_current2_input";
		}
	}
	...
	float delta_t=2; //segundos
	maxenergy = 0.0; 
	while (!end_inference){
 
		std::getline(file, str);
		float energy_= stoi(str);
		energy = energy + energy_*0.002; //tiempo sleep
		maxenergy = std::max(energy_, maxenergy);
		file.clear();
    ...
    std::vector<InferenceTrace>& trace, bool skipTransfers, float idleMs)
{
	...
	float maxenergy = 0;
	float maxenergycv = 0;
	energy_params ep, ep2;	
	ep.device="xavier";
	ep.sensor="gpu";
	ep2.device="xavier";
	ep2.sensor="cv";
	
	while (durationMs < warmupMs)
    {
        for (auto& s : iStreams)
        {
	...
	//NUESTRAS MEDIDAS
	bool end_inference=false;
	std::thread thread_obj(energy_measure, ep, std::ref(energy), std::ref(maxenergy), std::ref(end_inference));
	std::thread thread_obj2(energy_measure, ep2, std::ref(energy_cv), std::ref(maxenergycv), std::ref(end_inference));
	auto start = std::chrono::high_resolution_clock::now();
	int i = 0;

    for (i = 0; i < iterations || durationMs < maxDurationMs; ++i)
    {
        for (auto& s : iStreams)
        {
            if (!s->query(skipTransfers))
	...
	std::cout<<"TOTAL TIME: " << microseconds/1000/(i+1.0) << std::endl;
	std::cout <<"Energy GPU: " << energy/(i+1.0) << std::endl;
	std::cout <<"Energy CV: " << energy_cv/(i+1.0) << std::endl;	
	std::cout <<"Max energy: " << maxenergy/(i+1.0) << std::endl;		
	std::cout <<"Max energy CV: " << maxenergycv/(i+1.0) << std::endl;		
